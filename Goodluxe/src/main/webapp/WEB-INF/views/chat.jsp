            	...
            	if($("#userId").val()==wisper){
         		...
         		//본인이 아닐 경우에만 메세지 전달
            		...
            		$("#output").append("<i class='user icon'></i>"
            	    	+"<b style='color:blue'>[${member_id}]</b> : "
   	...
   	ws.onmessage = function(message) {
    	...
    	//메세지 
      	...
      	if(jsonData3.room !=null){
			...
			"전체 채팅방</b> (총 "+roomSplit[1]+"명 참여)</div></div>";
...
<!-- 세팅 부분 init -->
<input type="hidden" id="userId" value="${member_id}">  <!-- 유저아이디  -->
...
<div id="container">
	...
	<tr>
		<th><%=i+1 %></th>
		<th><%=vo.getChat_num() %></th>
		<th><%=vo.getMember_id() %>***</th>
		<th><%=vo.getChat_room() %></th>
	</tr>
	<% } %>
